#!/bin/bash
#
# Boltz Performance Benchmark - Pre-computed MSA Mode
# Bypasses MSA server to measure pure GPU inference time
#
# Usage: ./run_benchmark_precomputed_msa.sh [--quick] [--gpu-id N]
#
...
RESULTS_CSV="$RESULTS_DIR/benchmark_precomputed_msa_${TIMESTAMP}.csv"
SUMMARY_MD="$RESULTS_DIR/benchmark_precomputed_msa_${TIMESTAMP}.md"
...
run_test() {
    ...
    log "Running: $test_name (length=$length, batch=$batch, total=$total_residues residues) [PRE-COMPUTED MSA]"
    ...
    # Run prediction WITHOUT MSA server (using pre-computed MSA)
    local status="success"
    local input_dir_abs=$(realpath "$INPUT_DIR")
    local msa_dir_abs=$(realpath "$MSA_DIR")
    local output_abs=$(realpath "$test_output")
    local cache_abs=$(realpath "$CACHE_DIR")

    # Note: NOT using --use_msa_server, but binding MSA directory
...
main() {
    log "========================================"
    log "Boltz Performance Benchmark - Pre-computed MSA"
    log "========================================"
    log "Container: $CONTAINER"
    log "GPU ID: $GPU_ID"
    log "GPU: $(get_gpu_info)"
    log "Mode: $([ "$QUICK_MODE" -eq 1 ] && echo 'Quick' || echo 'Full')"
    log "MSA Mode: Pre-computed (no MSA server)"
    log "Tests: ${#TESTS[@]}"
    log "========================================"

    # Check that input files exist
    if [ ! -d "$INPUT_DIR" ] || [ -z "$(ls -A $INPUT_DIR)" ]; then
        echo "ERROR: Input directory is empty. Run setup_precomputed_msa.sh first."
        exit 1
    fi

    if [ ! -d "$MSA_DIR" ] || [ -z "$(ls -A $MSA_DIR)" ]; then
        echo "ERROR: MSA cache directory is empty. Run setup_precomputed_msa.sh first."
        exit 1
    fi
